import React, { useState, useRef } from 'react';
    ...
    function ItemManagement({ items, regions, onAddItem, onUpdateItem, onDeleteItem }) {
      ...
      const filteredItems = items.filter(item => item.regions && item.regions.includes(selectedRegion) && item.name.toLowerCase().includes(searchItem.toLowerCase()));
      ...
      const moveItemUp = async (item) => {
        ...
        if (currentIndex > 0) {
          const otherItem = filteredItems[currentIndex - 1];
